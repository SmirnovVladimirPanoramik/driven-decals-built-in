...
namespace SamDriver.Decal
{
    public static class StaticDecalMenuItem
    {
        ...
        {
            if (!Directory.Exists(sampleDecalsPath))
            {
                Debug.LogError($"Couldn't find Directory {sampleDecalsPath}");
                return null;
            }

            var path = Path.Combine(sampleDecalsPath, "Simple/Grid.asset");
            var decal = AssetDatabase.LoadAssetAtPath<DecalAsset>(path);
            if (decal == null) Debug.LogError($"Couldn't find Decal Asset at {path}");
            return decal;
        }
    }
}